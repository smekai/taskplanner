...
1. Call `taskplanner_board` with `include_tasks: true` for a complete grouped summary.
2. If the user requested a state or text filter, call `taskplanner_list` instead.
3. Show task ID, title, priority, and assignee; omit full descriptions unless requested.
4. Highlight tasks currently in In Progress.
