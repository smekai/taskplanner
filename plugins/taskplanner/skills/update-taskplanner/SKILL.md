...
2. Compare `taskplannerVersion` with `2.4.2`, stripping SemVer build metadata such as `+codex.*`.
...
6. Only after all writes succeed, set `taskplannerVersion` to `2.4.2` and save config. On failure, leave the previous value so a later invocation retries.
