...
2. Compare `taskplannerVersion` with `2.1.14`, stripping SemVer build metadata such as `+codex.*`.
...
6. Only after all writes succeed, set `taskplannerVersion` to `2.1.14` and save config. On failure, leave the previous value so a later invocation retries.
