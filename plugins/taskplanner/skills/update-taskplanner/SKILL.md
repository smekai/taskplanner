...
2. Compare `taskplannerVersion` with `2.2.10`, stripping SemVer build metadata such as `+codex.*`.
...
6. Only after all writes succeed, set `taskplannerVersion` to `2.2.10` and save config. On failure, leave the previous value so a later invocation retries.
