...
2. Create `.tasks/` and `config.json` with schema `version: 2`, `taskplannerVersion: "2.4.2"`, `readmeAttribution: true`, `idPrefix: "TASK"`, `nextId: 1`, the five default states, priorities P0-P4, empty tags, top insertion, required AI plans, and priority sorting.
...
5. Verify every required board file exists and the stored `taskplannerVersion` is `2.4.2`.
