...
- **npm** — `npm install @refined/taskplanner` for the MCP server on its own; see [`packages/mcp-server/README.md`](packages/mcp-server/README.md)
...
- **npm package** (`@refined/taskplanner`) for hosts that are not an editor plugin install — an agent runtime that wants the board as MCP tools resolves the server as a dependency instead of guessing at an extension path. It is the same server bundle as the plugin, built from the same sources.
